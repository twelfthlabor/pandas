...
- :meth:`DataFrame.to_html` with ``render_links=True`` and ``escape=True`` now escapes the generated ``href``, so a URL containing quotes or ampersands no longer produces malformed HTML (:issue:`66080`)
